                ...
                <tr key={req._id} className="hover:bg-emerald-50/30 transition-colors">
                  <td className="px-8 py-6 flex items-center gap-4">
                    <div className="w-16 h-16 rounded-2xl bg-slate-100 overflow-hidden border border-slate-200 shadow-inner">
                      <img src={req.images?.[0]?.url || "https://via.placeholder.com/100?text=No+Image"} alt="" className="w-full h-full object-cover"/>
                    </div>
                    <div>
...
function SellerAddProduct() {
  const [form,setForm]=useState({name:"",description:"",category:"",price:"",weight:"",quantity:"",sellerName:"",sellerEmail:"",sellerPhone:"",sellerAddress:""});
  const [images,setImages]=useState([null,null,null,null]);
  const [loading,setLoading]=useState(false);

  const handleChange = e => setForm({...form,[e.target.name]:e.target.value});
  const handleImageChange=(i,file)=>{const newImages=[...images];newImages[i]=file;setImages(newImages)};

  const handleSubmit=async e=>{
    e.preventDefault();
    setLoading(true);
    const selected=images.filter(i=>i);
    if(selected.length===0){toast.error("Please upload at least one image"); setLoading(false); return;}
    ...
    <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="lg:col-span-4 space-y-6">
        <div className="bg-white rounded-[2rem] p-6 shadow-xl border border-emerald-100">
          <h2 className="font-black text-slate-800 flex items-center gap-2 mb-6 text-xs uppercase tracking-widest"><ImageIcon className="w-4 h-4 text-emerald-500"/> Image Gallery</h2>
      ...
      <div className="lg:col-span-8 space-y-6">
        ...
        <div className="bg-white rounded-[2.5rem] p-8 shadow-xl border border-emerald-100">
          ...
          <button disabled={loading} type="submit" className="w-full mt-8 bg-emerald-600 hover:bg-emerald-700 text-white font-black py-6 rounded-2xl flex items-center justify-center gap-3 transition-all active:scale-[0.98] shadow-lg shadow-emerald-200">
